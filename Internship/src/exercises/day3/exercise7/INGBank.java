...
public class INGBank extends BankAccount {
    public INGBank(String bankName, double accountAmount, String ownerName, String ownerIdNo, String accountIBAN, double interest, Money currency) {
    ...
    public String withdraw(double withdrawnAmount) {
        if (withdrawnAmount < 50 || withdrawnAmount % 10 != 0.0) {
    ...
    public String deposit(double depositAmount) {
        if (depositAmount < 10 || depositAmount % 10 != 0) {
            return "The operation could not be processed. " +
                    "Please insert a positive value that is a multiple of 10";
        }
        if (depositAmount > 12_000) {
            return "The operation could not be processed." +
